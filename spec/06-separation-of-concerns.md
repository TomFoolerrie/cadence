...
Write scope is not just a documented convention — it is a hard boundary. **Cadence
runs on two tracks, and each enforces it with its own mechanism** (the spec below
is the same boundary expressed two ways):

- **Claude / Cowork track** — auto-generated `.claude/settings.json` files at each
  level. The scaffolders no longer inline these; `scripts/settings-gen.py <dir>
  <level>` writes the canonical per-level block, and the init scripts invoke it
  only when `CADENCE_TRACK=claude` (unset defaults to `claude`, so existing Cowork
  runs are unchanged). The JSON below is what `settings-gen.py` emits.
- **Pi track** — an in-process `tool_call` gate (`pi/extension/`, registered via
  the root `package.json` `pi` manifest). On `CADENCE_TRACK=pi` no `.claude/` is
  written; the gate reproduces the two deny rules below (write-above-root + the
  per-level script-gated file) **and additionally gates Bash** — a head whitelist
  + a destructive-pattern scan — which `settings.json` cannot do. It is fail-open
  (a gate bug never bricks a legitimate tool) because, unlike pi-harness, Cadence
  has no container boundary to fall back on.

**Task-level config** (Claude track: emitted by `settings-gen.py task`, invoked by `init-task.py`; at `{task}/.claude/settings.json`):
...
**Class-level config** (Claude track: emitted by `settings-gen.py class`, invoked by `init-class.py`; at `{class}/.claude/settings.json`):
...
**Root-level config** (Claude track: emitted by `settings-gen.py root`, invoked by `init-engagement.py`; at `{root}/.claude/settings.json`):
...
- `Bash` commands (including script invocations) are NOT restricted by `Write` permissions on the **Claude track** — the Write/Edit scope only applies to the agent's direct file editing tools. (The **Pi track** closes this gap: its gate also classifies Bash — a head whitelist + a banned-pattern scan — so e.g. a hand `mkdir` or `rm -rf` is rejected there too.)

**Why enforcement matters:** The dry run proved that instructions alone don't work — the agent bypassed `init-period.py` and hand-edited `.class.yaml` despite the skill explicitly saying to use scripts. Write scope enforcement makes it a hard boundary: the agent tries to `mkdir periods/dry-run/` directly and gets permission denied (on Claude, via the Write scope rejecting the edit/write tool; on Pi, via the Bash gate rejecting a non-whitelisted head), forcing it to use `init-period.py`. The agent tries to write `.class.yaml` and gets permission denied, forcing it to use `edit-class-yaml.py`. Scripts still work because they have their own filesystem permissions. This is defense in depth: instructions tell the agent what to do, write scope prevents it from doing anything else.
