...
- **Period advancement resets terminal states automatically.** `check-periods.py` runs on a schedule, reads each terminal task's `done_at` timestamp and `anchor` field, and resets tasks whose next period is due. Anchor dates use one-ahead logic (e.g., first Monday of the next period's month). A late-completion guard prevents immediate resets when a task finishes after its anchor has already passed — in that case, the reset pushes to the following cycle's anchor. This is the only mechanism that transitions out of a terminal state — it is a lifecycle event, not a within-period recovery. See `04-scaffolding.md` Section 6.6.
